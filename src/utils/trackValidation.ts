...
const validateSegmentConnection = (current: TrackSegment, next: TrackSegment): boolean => {
  ...
  return (
    Math.abs(current.endPoint.x - next.startPoint.x) < tolerance &&
    Math.abs(current.endPoint.y - next.startPoint.y) < tolerance
  );
};
...
const validateSegment = (segment: TrackSegment): string[] => {
  ...
  if (!segment.startPoint || typeof segment.startPoint.x !== 'number' || typeof segment.startPoint.y !== 'number') {
    errors.push(`Segment ${segment.id}: Invalid start coordinates`);
  }

  if (!segment.endPoint || typeof segment.endPoint.x !== 'number' || typeof segment.endPoint.y !== 'number') {
...
export const validateTrack = (track: Track): ValidationResult => {
  ...
  if (!Array.isArray(track.checkpoints) || track.checkpoints.length === 0) {
    ...
    for (let i = 0; i < orders.length; i++) {
      if (orders[i] !== i + 1) { // Changed to start from 1 instead of 0
        errors.push('Checkpoint order sequence is not continuous (should start from 1)');
        break;
      }
    }
  }
